...
WITH relevant_reaction_ids AS (
    ...
    SELECT REACTION_1 AS concept_id FROM {{ ref('gold_contingency_table') }}
    UNION
    SELECT REACTION_2 AS concept_id FROM {{ ref('gold_contingency_table') }}
)
...
FROM 
    {{ source('public', 'BRONZE_OUTCOME') }} AS src
JOIN
